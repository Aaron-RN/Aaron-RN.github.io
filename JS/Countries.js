...
function populateStatesAll(countryElementIndex, stateElementClass) {
    var allCountryElements = new Array();
    allCountryElements = document.querySelectorAll(".country");
    alert(countryElementIndex);

    var selectedCountryIndex = allCountryElements[countryElementIndex].selectedIndex;
    alert(selectedCountryIndex);

    var allStateElements = new Array();
    allStateElements = document.getElementsByClassName(stateElementClass);
    
    var state_arr = s_a[selectedCountryIndex].split("|");
    
    for (var i = 0; i < allStateElements.length; i++){
        allStateElements[i].length = 0; // Fixed by Julian Woods
        allStateElements[i].options[0] = new Option('Select State', '');
        allStateElements[i].selectedIndex = 0;
        for (var e = 0; e < state_arr.length; e++) {
            allStateElements[i].options[allStateElements[i].length] = new Option(state_arr[e], state_arr[e]);
        }
    }
}

function populateCountriesAll(countryElementClass, stateElementClass) {
    // given the class of the <select> tag as function argument, it inserts <option> tags
    var allCountryElements = new Array();
    allCountryElements = document.getElementsByClassName(countryElementClass);
    for (var i = 0; i < allCountryElements.length; i++){
        allCountryElements[i].length = 0;
        allCountryElements[i].options[0] = new Option('Select Country', '-1');
        allCountryElements[i].selectedIndex = 0;
        for (var e = 0; e < country_arr.length; e++) {
            allCountryElements[i].options[allCountryElements[i].length] = new Option(country_arr[e], country_arr[e]);
        }
    }
    ...
    if (stateElementClass) {
        for (var i = 0; i < allCountryElements.length; i++){
            var selectedCountryElement = allCountryElements[i];
            var querySelectorAllIndex = i;
            selectedCountryElement.onchange = function () {
                alert(selectedCountryElement.selectedIndex);
                populateStatesAll(querySelectorAllIndex, stateElementClass);
            };
        }
    }
}